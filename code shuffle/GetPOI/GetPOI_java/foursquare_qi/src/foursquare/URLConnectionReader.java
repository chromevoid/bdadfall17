...
public class URLConnectionReader {
    ...
    static String clinet_id = "Z5WDNPRUNZULW2WFQPB2YUVFJHOTTMEMYJ32A2CML3CLC25I";
    static String client_secret = "V1PLZSK1ZFYCCLU2ZVGA4DXC1SEACU2KNNTQOSKM3UCJJPXP";
    ...
    public static void main(String[] args) throws Exception {
        ...
        out = new FileWriter(timeStamp + ".csv", true);
        // set start time
...
//        poi(chicago_lat_min, chicago_lat_max - 93 * chicago_delta_lat, chicago_lon_min, chicago_lon_max, 20, 100);
    ...
    private static void poi(double lat_min, double lat_max, double lon_min, double lon_max, int row, int col) throws Exception {
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = 0; j < col; j++) {
                System.out.println("col#: " + j + "\t" + System.currentTimeMillis());
                ...
                if ((now - startTime) < 24 * 3600000) {
                    if (requestCount >= 90000) {
                        long wait = 24 * 3600000 - (now - startTime);
                        try {
...
//                     Statement stmt = conn.createStatement()
                    ...
                    if (items.size() < 50) {
                        for (JsonObject item : items.getValuesAs(JsonObject.class)) {
                            ...
                            JsonValue zipcode = location.get("postalCode");

                            JsonArray categories = venue.getJsonArray("categories");
                            JsonObject category_obj = categories.getJsonObject(0);
                            String category = category_obj.getString("name");
                            String sub_category = category_obj.getString("shortName");

                            String zipcodestr = zipcode == null ? "" : zipcode.toString().replace("\"", "");

                            String[] record = new String[]{
                                    lat.toString(), lng.toString(), zipcodestr,
                                    name, category, sub_category
                            };
                            dumpRecordToFile(String.join(",", record) + "\n");
                        }
    ...
    static void dumpRecordToFile(String str) throws IOException {
        out.write(str);
        out.flush();
    }
